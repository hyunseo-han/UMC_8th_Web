...
function usePostLike() {
  return useMutation({
    ...
    // },
    // // 요청이 끝난 후 항상 실행됨(OnSuccess, onError 이후에 실행)
    // // 로딩 상태를 초기화할 때 조금 유용
    // onSettled: (data, error, variables, context) => {},
    // onMutate -> API 요청 이전에 호출되는 친구
    onMutate: async (lp) => {
      // 1. 이 게시글의 관련된 쿼리를 취소 (캐시된 데이터를 새로 불러오는 요청)
      ...
      // 3. 게시글 데이터를 복사해서 NewLpPost라는 새로운 객체를 만듦
      // 복사하는 가장 큰 이유는 나중에 오류가 발생했을 때 이전 상태로 되돌리기 위해서라고 생각하면 됨
      const newLpPost = { ...previousLpPost };

      // 4. 게시글에 저장된 좋아요 목록에서 현재 좋아요의 위치를 찾아야 함
